...
class DynamicPuffPlume:
    ...
    def __init__(
        self,
        seed: int = 0,
        source_position: tuple[float, float] = (2.8, 1.0),
        dt: float = 0.2,
        gas_field_mode: str = "puff",
        wind_speed_range: tuple[float, float] = (0.28, 0.50),
        ...
        self.wind_speed_std = 0.05
        self.wind_dir_noise_std = 0.03
        self.wind_relaxation = 0.97
        self.wind_direction_limit = math.radians(15.0)
        self.wind_speed_clip = (
            max(0.2, self.wind_speed_range[0] - 0.3),
            max(self.wind_speed_range[1] + 0.3, 1.6),
        )

        self.source_core_sigma = 0.36
        self.source_core_weight = 0.28
        self.puff_release_per_step = 1
        self.puff_init_mass = 1.0
        self.puff_init_sigma = 0.18
        self.diffusion_rate = 0.16
        self.decay_rate = 0.020
        self.turbulence_std = 0.018
        self.max_puff_age = 12.0
        self.min_puff_mass = 0.01
        self.max_puffs = 140
        self.puff_bounds_margin = 1.5
        self.puff_warmup_steps = 28
    ...
    def metadata(self) -> dict[str, Any]:
        ...
        return {
            ...
            "obstacles": np.empty((0, 4), dtype=np.float32),
            "gas_field_mode": "puff",
    ...
    def is_occupied(self, x: float, y: float) -> bool:
        return x <= WORLD_MIN or x >= WORLD_MAX or y <= WORLD_MIN or y >= WORLD_MAX
    ...
    def _sample_wind(self) -> None:
        ...
        if self.wind_sampling_mode == "fixed":
            ...
            return
        self.wind_direction_base = float(self.rng.uniform(0.0, 2.0 * np.pi))
    ...
    def _emit_puffs(self) -> None:
        ...
        for _ in range(self.puff_release_per_step):
            self.puffs.append(
                {
                    "x": float(self.source_x),
                    "y": float(self.source_y),
                    "mass": float(self.puff_init_mass),
                    "sigma": float(self.puff_init_sigma),
                    "age": 0.0,
                }
            )
    ...
    def _advance_puffs(self) -> None:
        ...
        for puff in self.puffs:
            puff["age"] += self.dt
            puff["x"] += wind_x * self.dt + float(
                self.rng.normal(0.0, self.turbulence_std)
            )
            puff["y"] += wind_y * self.dt + float(
                self.rng.normal(0.0, self.turbulence_std)
            )
            puff["sigma"] += self.diffusion_rate * self.dt
    ...
    def _puff_concentration(self, x: float, y: float) -> float:
        ...
        for puff in self.puffs:
            ...
            total += puff["mass"] * math.exp(
                -(dx * dx + dy * dy) / (2.0 * sigma_sq)
            ) / (2.0 * math.pi * sigma_sq)
        return total
    ...
    def _sample_source_position(self) -> tuple[float, float]:
        """随机采样气源位置；当前无障碍物，只需避开边界。"""
        while True:
            x = self.rng.uniform(
                WORLD_MIN + self.source_clearance,
                WORLD_MAX - self.source_clearance,
            )
            y = self.rng.uniform(
                WORLD_MIN + self.source_clearance,
                WORLD_MAX - self.source_clearance,
            )
            if self.is_position_valid(x, y, margin=self.source_clearance):
    ...
    def reset(self, seed: int | None = None) -> None:
        if seed is not None:
            self.rng = np.random.default_rng(seed)
        self._sample_wind()
        self.source_x, self.source_y = self._sample_source_position()
        self._warmup_puffs()
...
class WhiskerOnlyPuffEnv(gym.Env):
    ...
    def __init__(self, config: dict[str, Any] | None = None) -> None:
        ...
        self.plume = DynamicPuffPlume(
            seed=seed,
            source_position=tuple(cfg.get("source_position", (2.8, 1.0))),
            dt=float(cfg.get("dt", 0.2)),
            gas_field_mode=str(cfg.get("gas_field_mode", "puff")),
            wind_speed_range=tuple(cfg.get("wind_speed_range", (0.28, 0.50))),
        ...
        self.whiskers = DualWhiskerSampler(
            length=float(cfg.get("whisker_length", 0.45)),
    ...
    def reset(
        ...
        *,
        ...
        self.robot_state = self.sample_robot_pose()
        return self._observe()

    def step(self, action: Any) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        left_sector, right_sector = self._decode_action(action)
        self.plume.advance()
        whisker_state = self.whiskers.step(left_sector, right_sector, self.robot_state)
    ...
    def sample_robot_pose(self) -> RobotState:
        while True:
            x = self.rng.uniform(WORLD_MIN + 0.5, WORLD_MAX - 0.5)
            y = self.rng.uniform(WORLD_MIN + 0.5, WORLD_MAX - 0.5)
            if math.hypot(x - self.plume.source_x, y - self.plume.source_y) < 1.0:
                continue
