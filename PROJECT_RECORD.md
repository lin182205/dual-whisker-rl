...
## 26. Mobile 二维仿真雷达与矩形障碍闭环

为后续在移动气味搜索场景中加入障碍物，先建立纯 Python/NumPy 的二维感知与碰撞闭环，不引入 ROS、Gazebo 或点云依赖。实现参考 Velodyne 导航环境的“前向视场分扇区、每束保留最近距离”抽象，但直接对二维轴对齐矩形做解析式 ray-AABB 求交。

### 26.1 雷达与障碍几何

新增 `SimulatedLidar2D`，默认前向 180°、12 束、量程 0.6m。12 束位于等宽 15° 扇区中心；每束同时检测矩形障碍和世界边界，无命中时返回最大量程。策略观测使用 `distance / max_range` 归一化到 `[0, 1]`。默认噪声为 0，但保留 `lidar_noise_std` 配置入口。

障碍统一表示为 `[[xmin, xmax, ymin, ymax], ...]`，初始化时校验二维形状、有限值、正面积和世界边界。该数组由地图 metadata、初始位姿采样、雷达和碰撞逻辑共享。机器人碰撞体为半径 0.08m 的圆；移动后若与矩形相交，完整位姿回滚到动作前状态，episode 立即以 `collision` 终止并施加默认 `collision_penalty=25`。世界边界仍沿用原中心点 `out_of_bounds` 语义，避免改变旧无障碍实验。

本阶段障碍**不阻挡、反射或改变 puff 羽流**。障碍只约束气源/机器人位置并参与雷达和机器人碰撞；若以后研究障碍绕流，必须单独修改羽流模型并重新运行间歇性诊断和奖励量级标定。

### 26.2 观测兼容与示例场景

雷达扫描始终写入 `info`、trajectory 和地图 metadata，包括米制/归一化12束距离、最近距离、机器人半径和束角。`include_lidar_observation=false` 为全局默认，因此原 16 维 mobile 观测及旧 checkpoint 不受影响；开启后在现有字段末尾追加12维，单帧变为28维，history=20 时由320维变为560维。

新增 `configs/mobile_obstacles.yaml`：固定气源 `(-0.8, 0)`、两块保留上下绕行通道的矩形障碍，并默认开启雷达观测。该配置必须从新模型开始训练。训练、评估和可视化入口在 checkpoint 维度不匹配时会同时报告 `include_blank_age_observation` 与 `include_lidar_observation`，避免把所有不兼容误归因于 blank_age。

移动可视化现在绘制障碍矩形、圆形机器人 footprint 和当前12束雷达射线；信息框显示最近雷达距离，碰撞使用独立终止标签。训练 metadata 同步记录雷达参数、障碍列表、机器人半径、观测开关及碰撞惩罚。

### 26.3 连通性验证（不是避障能力结论）

- `diagnose_mobile_lidar.py` 的确定性检查通过：世界边界命中、正前方左右对称、随机器人旋转、0.6m 量程裁剪、非法障碍拒绝、碰撞位姿回滚和单次 `-25` 惩罚均符合预期。
- 默认/障碍配置观测分别为16/28维且满足 Gymnasium Box；history=20 时为320/560维；采样起点和气源均不落入障碍安全区。
- 障碍配置的30-step 随机可视化成功生成 PNG/GIF，人工检查障碍、雷达端点和机器人 footprint 对齐。
- 256-step、1 env、MLP PPO smoke 跑通；metadata 为 base/stacked=`28/56`、12束、2个障碍、半径0.08m、碰撞惩罚25，训练期间能够产生碰撞终止 episode。

这些结果只证明几何、观测、碰撞、日志和训练链路连通，不能证明策略已经学会避障。正式实验应从障碍配置新开 run，并比较碰撞率、成功率、路径长度和气味重捕获指标。

## 27. Mobile 场地扩展到 4m × 4m + 碰撞半径 0.20m

为后续布置更多障碍并保留可绕行空间，`MobileWhiskerPuffEnv` 默认 `world_half` 从 1.0 改为 2.0，即世界边界由 `[-1,1]²` 扩展为 `[-2,2]²`。固定场景默认气源随边界移动到 `(-1.8, 0)`；随机场景的气源距离、机器人下风距离和横风偏移原本按 `world_half` 成比例计算，因此自动扩展到新尺度。机器人圆形碰撞半径由 0.08m 改为 0.20m，起点合法性检查也自动采用新半径。

`configs/mobile_obstacles.yaml` 同步改为 4m × 4m：固定气源为 `(-1.8,0)`，两块示例矩形按场地比例扩展，并保留上下绕行通道。雷达仍为前向 180°、12 束、0.6m 量程；相对 0.20m 碰撞半径仍提供约 0.4m 的最大提前感知距离。

场地扩大后，按世界对角线计算的正辅助奖励理论上界由 12.4853 增至 20.9706。为保持 `progress_reward_scale=3`、单步时间惩罚和 `goal_bonus=50` 不变，仅做以下终止安全适配：辅助奖励上界不得超过 `goal_bonus/2=25`；默认 `oob_penalty` 和 `collision_penalty` 从25提高到35。最大时间成本仍为12，因此终止惩罚理论下界为 `12+20.9706=32.9706`，35满足约束。旧模型虽然观测维度不变，但场景分布和终止代价已变化，正式训练应新开 run。

为让气味 puff 覆盖扩展后的下风距离，mobile 默认 `max_puff_age` 从16s延长到24s，`puff_warmup_steps` 从80提高到120，使 reset 时的预热时长同为24s，避免寿命已经延长但远场仍因预热不足形成空场；固定基座 `WhiskerOnlyPuffEnv` 的默认羽流参数不变。训练 metadata 新增实际世界边界、puff 寿命和预热步数。修改前后的标准间歇性诊断均需确认固定基座 whiff/blank 统计没有漂移。

### 27.1 连通性与量级验证

- 默认和障碍环境分别保持16/28维观测，世界边界均为 `[-2,2]²`，碰撞半径为0.20m；确定性雷达/碰撞诊断和 Gymnasium checker 通过，每步 reward 与8个分量之和误差小于 `1e-12`。
- 标准固定基座羽流在修改前后统计完全一致：中心线 intermittency 0.245、whiff 0.83s、blank 2.53s，说明 mobile 覆盖参数没有污染 whisker-only 基线。
- 固定障碍场景30个 seed 的 reset 瞬时最大触须浓度均值由16秒预热时的0.0190提高到24秒预热时的0.0466；远场仍以低浓度/blank 起步，符合需要主动搜索而非初始常通的目标。
- 4m × 4m PNG/GIF 检查通过，边界、缩放后的障碍、0.20m footprint 和0.6m雷达射线对齐；短 PPO smoke 可正常训练并写出新 metadata。以上仅证明连通性，不代表新场地已经学会避障或找源。

## 28. D2Q9 LBM 障碍风场与 Puff 羽流耦合

§26 明确保留了“障碍不影响羽流”的阶段性限制，但这会让气味直接穿墙，不适合作为正式障碍训练动力学。本阶段用纯 NumPy 二维 D2Q9 Lattice Boltzmann Method 生成定性稳态风场，再让原有 Gaussian Puff 沿局部风速运动。障碍现在同时参与雷达、机器人碰撞、LBM 固体边界和气味遮挡；§26.1 的旧限制从本节起被取代。

### 28.1 稳态 D2Q9 风场

新增 `SteadyLBMWindField2D`：默认 `129×129` 核心网格、外围12格缓冲，使用单松弛时间 BGK（`tau=0.60`）、低马赫格子速度0.04和矩形固体 bounce-back。平均风向按10°量化，最多产生5°误差；缓存键包含地图、障碍、网格、LBM 参数和量化方向，同一进程内的 `DummyVecEnv` 可直接复用。求解最多2500步；稳定场达到残差阈值便提前结束，方柱尾流未静止时对最后400步速度做时间平均。

这不是高 Reynolds 数空气 CFD：二维障碍视为不可从顶部越过的完整墙体，LBM 只提供低马赫平均绕流。气体是被动标量，不反向作用于风场，也不模拟浮力、温湿度、壁面吸附和化学反应。当前风向 OU 不触发昂贵的 LBM 重算，而作为 Puff 的缓慢横向阵风保留。

### 28.2 Puff 输运和墙体语义

有障碍时，Puff 批量双线性插值 LBM 速度，使用 RK2 和按 Puff 独立的平均场 CFL 子步推进；Gaussian 长轴跟随局部速度方向。由局部速度亏损和涡量构造 `[1,3]` 尾流湍流倍率，只增强原有横向 OU 扰动和横向扩散。swept segment–AABB 是数值保险：若一个子步试图进入墙体，法向位移被截断、切向位移保留，因此 Puff 中心和运动线段均不能直接穿墙。

仅阻止中心穿墙仍不够，因为 Gaussian 尾部可能跨过矩形。浓度查询因此对 Puff—采样点线段做障碍遮挡，默认 `wall_transmission=0`；障碍内部固定为背景浓度。`obstacle_flow.enabled=false` 可恢复旧穿墙模型做消融；无障碍时完全绕过 LBM 和新输运路径，不额外消耗随机数。

`configs/mobile_obstacles.yaml` 显式记录完整 `obstacle_flow` 参数。地图 metadata 与训练 `run_metadata.json` 新增网格间距、格子黏性、Mach 数、请求/量化方向、缓存命中、迭代数、残差、密度范围和是否时间平均。策略观测、动作、雷达、碰撞、奖励和 checkpoint 张量维度均未改变。

### 28.3 诊断、显示和性能

新增 `scripts/diagnose_obstacle_plume.py`，确定性覆盖无障碍随机序列回归、固体零速度、水平对称、90°旋转、尾流速度亏损、方向缓存、Puff 防穿墙和 Gaussian 墙体遮挡；同时生成风速/流线、涡量、尾流湍流倍率、瞬时/平均浓度和横风截面图。两障碍诊断图显示主墙前分流、墙后成对回流涡和浓度阴影，Puff 从上下通道绕行后才进入下游。

4m 场地的移动可视化浓度网格默认由100提高到200，并使用 bicubic 插值，恢复约2cm/格的显示采样；物理 LBM 网格仍独立为129。根据后续显示要求，常规 PNG/GIF 不再绘制雷达射线和机器人圆形碰撞轮廓，也不绘制风场箭头；风场只出现在独立诊断图。

默认两障碍配置实测：首次 LBM reset 约6.996s，缓存命中 reset 约0.192s；2500步后的密度范围为 `[0.9872, 1.0072]`、末次残差约0.00377，使用尾段时间平均。缓存后500步环境基准约2.991ms/step，相对修改前约1.027ms为2.91倍，达到不超过3倍的目标；推进后无 Puff 位于障碍内部。

### 28.4 回归与训练 smoke

- `diagnose_obstacle_plume.py` 和原 `diagnose_mobile_lidar.py` 均通过；默认/障碍观测保持16/28维，history=20 仍为320/560维。
- 无障碍3000步间歇性统计保持完全一致：中心线 intermittency 0.245、whiff 0.83s、blank 2.53s、mean concentration 0.064，说明固定基座基线未漂移。
- 200分辨率、20步移动 PNG/GIF 生成成功，人工确认羽流边缘平滑、障碍遮挡清楚，且没有雷达射线或圆形 footprint。
