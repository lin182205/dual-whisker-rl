...
## 40. Mobile GRU/PPO 等价工程加速

在不改变 PPO 超参数、观测、奖励和随机过程的前提下，对 800 万步独立训练入口完成五项
工程优化。默认周期评估由每 2,000 全局步、10 局改为每 100,000 步、50 局；checkpoint
由每 10,000 步改为每 100,000 步。这样将完整训练中的同步评估触发次数从约 4,000 次降至
约 80 次，同时以更多单次样本稳定成功率估计；两个频率仍可通过命令行覆盖。

动态 puff 热点改为在释放时缓存固定方向的正余弦，每个环境步只计算一次衰减因子，并用
一次 puff 遍历同时计算左右触须浓度。训练环境新增运行时轻量模式：不保存仅供可视化的逐步
trajectory，跨进程 `info` 仅保留奖励分量、blank/重捕获诊断和 `is_success`；普通环境、离线
评估和可视化入口继续保留完整 trajectory/info。训练脚本在导入 NumPy/PyTorch 前将
`OMP_NUM_THREADS`、`MKL_NUM_THREADS`、`OPENBLAS_NUM_THREADS`、`NUMEXPR_NUM_THREADS`
统一设为 1，避免 12 个 worker 各自再次扩线程及无效 OMP 值触发 `libgomp` 警告。实际设置、
精简字段和采样模式均写入 `run_metadata.json`。

等价性验证采用固定种子 4242、确定动作序列运行 400 步，优化前后包含完整观测、奖励、
终止状态、左右原始浓度、风场和 puff 数量的 JSON 摘要 SHA-256 均为
`334a07727b605b2cc9486712cbf040faf1210392be77ba486ff4bf26044bf343`。羽流间歇性诊断的
四个横风测点在优化前后逐项一致（intermittency 分别为 0.245、0.253、0.244、0.203）。
本机相同 cProfile 口径的 3,000 步单环境基准由约 3.92 秒降至约 2.14 秒，约 1.83 倍，
函数调用数由约 784 万降至约 145 万；不挂 profiler 的优化后参考耗时约 1.14 秒。完整/精简
...
审计所有活跃的训练/评估入口后，发现静态高斯场的 Joint PPO 与 Fixed-whisker DQN
已共同读取 configs/default.yaml，E4 的训练和评估已共用 manifest 配置；但 Mobile
独立训练和固定场景复评默认仍取低层环境的 ±1.0，而单独的 Mobile PPO 评估被设置为
±2.5。触须采样对比评估直接构造环境字典，不能读取触须训练的 YAML。

新增 configs/mobile_whisker.yaml，将独立 Mobile 训练、单模型评估、固定场景复评及
相关可视化/注意力分析的默认场景统一为动态 puff、随机初始化、边界 [-2.5, 2.5]、
风速范围 [0.12, 0.20] 和 max_steps 400。新增 configs/whisker_only.yaml，
把固定机器人实验原有的 ±0.5、plume_edge、hardware 观测和 300 步默认值显式写入
配置，触须训练与采样策略评估共同读取。静态高斯场维持其独立的 10×10 场景，
不能与固定机器人或 Mobile 场景混为同一个环境。

各入口保留 --config 覆盖；触须采样评估的 --steps、--init-pose-mode 改为仅在
显式传入时覆盖 YAML。E4 base_config 不再无条件覆盖 YAML 中的风速范围，显式
--max-steps 则同步写回有效环境配置，确保训练环境和评估上限一致。各评估输出记录
有效环境配置。Mobile 固定场景库的默认文件名包含配置摘要、基础种子及局数，复用时
额外核对基础种子，避免旧 2×2 题库被 5×5 评估静默复用。

验证：10 个受影响脚本通过语法检查，E4/移动场景初始化 17 项单元测试通过；
Mobile 默认训练/两种评估解析到同一配置文件，环境边界为 ±2.5；触须训练与采样
评估解析到同一配置文件，边界为 ±0.5。Mobile 1 局评估、64 步 GRU/PPO smoke
训练、2 局固定场景库准备及触须固定扇区短评估均跑通；训练 metadata 和评估指标
记录 [-2.5, 2.5]。E4 自定义风速 [0.14, 0.18] 与步数 350 均被有效配置采用，
固定场景库拒绝不同基础种子的复用。完整 unittest discover 共运行 39 项，另有 5 项
